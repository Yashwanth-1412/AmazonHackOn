...
export default function BottomNav() {
  ...
  if (pathname === "/checkout" || pathname.startsWith("/payment")) {
    return null
  }
            ...
            >
              ...
              {badge && count > 0 && (
                <motion.span
                  ...
                  className="absolute -top-1 -right-1 bg-[#ff9900] text-white text-[8px] font-black rounded-full w-4 h-4 flex items-center justify-center"
                >
                  {count > 9 ? "9+" : count}
                </motion.span>
              )}
              <span
                className={`text-[10px] font-medium ${active ? "text-[#232f3e] font-semibold" : "text-[#888c8c]"}`}
              >
                {label}
              </span>
              {active && (
                <motion.div
                  ...
                  className="w-4 h-0.5 bg-[#ff9900] rounded-full mt-0.5"
